// is going to hold application module and route
angular.module("seller", ["ngRoute", "angular-jwt"]).config(config).run(run);

function config($routeProvider, $httpProvider, $locationProvider) {
  // $locationProvider.hashPrefix("");
  $routeProvider
    .when("/", {
      templateUrl: "angular-app/welcome/welcome.html",
      access: {
        restricted: false,
      },
    })

    .when("/sellers", {
      templateUrl: "angular-app/seller-list/sellers.html",
      controller: "SellersController",
      controllerAs: "vm",
      access: {
        restricted: false,
      },
    })
    .when("/seller/:id", {
      templateUrl: "angular-app/seller-display/seller.html",
      controller: "SellerController",
      controllerAs: "vm",
      access: {
        restricted: false,
      },
    })
    
    .when("/delete/seller/:id", {
      templateUrl: "angular-app/seller-delete/seller-delete.html",
      controller: "DeleteSellerController",
      controllerAs: "vm",
    })
  .when("/register", {
    templateUrl: "angular-app/register/register.html",
    controller: "RegisterController",
    controllerAs: "vm",
  }).
  when("/games/search/:title", {
    templateUrl: "angular-app/search-display/search-display.html",
    controller: "GameSearchController",
    controllerAs: "vm",
    access: {
       restricted: false 
      },
  })
    .when("/profile", {
      templateUrl: "angular-app/profile/profile.html",
      controllerAs: "vm",
      access: {
        restricted: true,
      },
    })

    .otherwise({
      redirectTo: "/",
    });
}

function run($rootScope, $location, $window, AuthFactory) {
  $rootScope.$on(
    "$routeChangeStart",
    function (event, nextRoute, currentRoute) {
      if (
        nextRoute.access !== undefined &&
        nextRoute.access.restricted &&
        !$window.sessionStorage.token &&
        !AuthFactory.isLoggedIn
      ) {
        event.preventDefault();
        $location.path("/");
      }
    }
  );
}
